...
def display_count(img, show_line=True):
    global object_counter, object_counter1
    height, width, _ = img.shape
    if show_line:
        cv2.line(img, line[0], line[1], (46,162,112), 3)
    ...
    for idx, (key, value) in enumerate(object_counter1.items()):
        ...
        cv2.line(img, (width - 450,25), (width,25), [85,45,255], 40)
        cv2.putText(img, f'Number of Objects Entering', (width - 500, 35), 0, 1, [225, 255, 255], thickness=2, lineType=cv2.LINE_AA)
        cv2.line(img, (width - 180, 65 + (idx*40)), (width, 65 + (idx*40)), [85, 45, 255], 30)
        # print(f"cv2.putText(img, {cnt_str})")
        cv2.putText(img, cnt_str, (width - 180, 75 + (idx*40)), 0, 1, [255, 255, 255], thickness = 2, lineType = cv2.LINE_AA)

    for idx, (key, value) in enumerate(object_counter.items()):
        cnt_str1 = str(key) + ":" +str(value)
        cv2.line(img, (20,25), (500,25), [85,45,255], 40)
        cv2.putText(img, f'Numbers of Objects Leaving', (11, 35), 0, 1, [225, 255, 255], thickness=2, lineType=cv2.LINE_AA)    
        cv2.line(img, (20,65+ (idx*40)), (150,65+ (idx*40)), [85,45,255], 30)
