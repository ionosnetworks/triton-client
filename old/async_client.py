...
def completion_callback(infer_status, frames, output_dict, result, error):
    if error:
        infer_status.put(error)
    else:
        batch_end = time.time()
...
if __name__ == '__main__':
    ...
    for frames in frame_generator(cap, FLAGS.batch_size):
        ...
        output = model.async_infer(input_image_buffer, partial(completion_callback, infer_status, frames, output_dict), request_id=str(sent_count))
